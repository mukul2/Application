...
import 'package:flutter_app_tv/ui/serie/series_short_detail_mini.dart';
...
class _SeriesState extends ResumableState<Series> {
  ...
  void _getGenres()  async{
    ...
    print("fire "+qs.docs.length.toString());

    for(int i = 0 ; i < qs.docs.length ; i++){

      try{
        print("loop "+i.toString());
        print(qs.docs[i].get("name"));
        String data = qs.docs[i].get("list");
        List show =  convert.jsonDecode(data);


        print("show count  "+show.length.toString());
        ...
        List<Poster> oneCategoryShows = [];
        for(int j = 0 ; j < show.length ; j++){
          //String m3uFile = SERVER+":$PORT"+"/"+show[j]["stream_type"]+"/"+EMAIL+"/"+PASSWORD.toString() +"/"+show[j]["stream_id"].toString()+"."+show[j]["container_extension"];
          // print(m3uFile);

          Poster poster1 = Poster(id:show[j]["series_id"],
              title: show[j]["name"],
              type: "type",
              label: null,
              sublabel: null,
              ...
              rating:0.0,
              image: show[j]["cover"]??"https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png",
              cover: show[j]["cover"]??"https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png",
              trailer: null,
          ...
          oneCategoryShows.add(poster1);
        }



        allSeries.add(oneCategoryShows);

        genres.add(Genre(id: i+1,title: qs.docs[i].get("name")));
      }catch(e){
        print(e);
        print(qs.docs[i].id);
      }
  ...
  void _getList()  async{
    ...
    page =0;
    _showLoading();
    series.addAll(allSeries[_selected_genre]);
    _showData();
            ...
            children: [
              ...
              if(series.length>0 && !_visibile_loading && !_visibile_error)
                AnimatedPositioned(
                    ...
                    child: SeriesShortDetailMiniWidget(movie :  series[_focused_poster])
                ),
              Positioned(
                top:  10,
                left: 45,
