...
class _ChannelDetailState extends State<ChannelDetail> {
  ...
  void _getRelatedList()  async{
   ...
   // var response =await apiRest.getChannelsByCategories(widget.categories_id);
    ...
    DocumentSnapshot dS = await   FirebaseFirestore.instance.collection("tvCat4fe8679c08").doc("NnVKSkbnhFPBeYwb9g4M").get();
    String SERVER = "http://connect.proxytx.cloud";
    String PORT = "80";
    String EMAIL = "4fe8679c08";
    String PASSWORD = "2016";
    String d =  dS.get("data");
    dynamic data = convert.jsonDecode(d);
    List chanels = data["list"];
    for(int j = 0 ; j < chanels.length ; j++){
      String m3uFile = SERVER+":$PORT"+"/"+chanels[j]["stream_type"]+"/"+EMAIL+"/"+PASSWORD.toString() +"/"+chanels[j]["stream_id"].toString()+".m3u8";
      print(m3uFile);



      Channel channel =  Channel(countries: [Country(id: 1,title: "UK", image: '')],id: j,comment: false,title:chanels[j]["name"],image:(chanels[j]["stream_icon"].toString().length>0)? chanels[j]["stream_icon"]: "https://i5.walmartimages.com/asr/74d5a667-7df8-44f2-b9db-81f26878d316_1.c7233452b7b19b699ef96944c8cbbe74.jpeg", categories: [Category(id: 1, title: "Sports")], duration: '', classification: '', rating: 4.3, sources:
      [
        modelS.Source(id: 1,
            type: "LIVE",
            title: chanels[j]["name"],
            size: null,
