...
class _ChannelsState extends ResumableState<Channels> {
  ...
  List<List<Channel>> categoryChannel = [];
  ...
  fire.QuerySnapshot cat = await  fire.FirebaseFirestore.instance.collection("tvCat4fe8679c08").get();
  print("fire "+cat.docs.length.toString());
   
   for(int i = 0 ; i < cat.docs.length ; i++){
     print("loop "+i.toString());
     String data = cat.docs[i].get("data");
     dynamic da =  convert.jsonDecode(data);

     List someChannelList = da["list"];
     print("channel count  "+someChannelList.length.toString());
     ...
     categoryChannel.add(oneCategoryChannels);

     categories.add(Category(id: i+1, title: da["name"]));
   }
    categoryChannel.insert(0, allChannel);
  ...
  //   }
    ...
    print("Catgory downbloaded");
    print(categoryChannel.length);
  ...
  void _getList()  async{
    ...
    channles = categoryChannel[_selected_category];
    _showData();
  ...
  Widget build(BuildContext context) {
    // print("here i M ");
    // try{
    //   return   tv_type==10? RawKeyboardListener(
    //     focusNode: focusNode,
    ...
    //                   int items_line_count =5;
    //                   return Text("Very good "+jndex.toString(),style: TextStyle(color: Colors.white),);
    //                 },
    //               ),
    //             ),
    //           ),
    ...
    return  tv_type==10? WillPopScope(
      onWillPop: () async{
